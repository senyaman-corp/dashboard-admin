      ...
      <client-only>
        ...
        <template #column-6="props">
          ...
          <ul id="dropdown" class="dropdown-menu p-2" aria-labelledby="dropdown">
            <li class="dropdown-item" v-if="user !== null && user.jabatan == 'Superadmin'">
              <a type="button" class="btn btn-default" @click="deleteUser(props.rowData.id)">
                <i class="fas fa-trash-alt me-2"></i>Delete</a>
            </li>
            <li class="dropdown-item">
              <a type="button" class="btn btn-default" @click="viewUser(props.rowData.id)">
...
const options = $dataTableOptions(config.public.baseUrl + 'users/list', authStore.getToken);
options.columnDefs = [
...
const deleteUser = async (id) => {  
  try {
    const { data , status } = await $fetch(`${config.public.baseUrl}users/delete`, {
      lazy: true,
      method: "POST",
      headers: {
        Authorization: "Bearer " + authStore.getToken,
