  ...
  <div class="card">
    <div class="card-body p-3 p-sm-4">
      <div class="row justify-content-between align-items-start">
        ...
        <div class="col-md-2">
         <InputWithCombobox :options="rooms" v-model="room_id" placeholder="Room Number" @update:model-value="filterByRoom(room_id)"/>
        </div>
        <div class="col-md-1 text-end">
          <NuxtLink :to="'/housekeeping/add-check-list'" class="btn btn-primary btn-md" variant="primary">Tambah</NuxtLink>
        </div>
      </div>
...
<script setup>
  ...
  const options = $dataTableOptions(config.public.baseUrl + 'housekeeping/checklist-room', authStore.getToken,body.value);
  options.columnDefs = [{
      targets: [0],
    ...
    $swal.fire({
      ...
      if (result.isConfirmed) {
        const {
          data,
          status
        } = await $fetch(`${config.public.baseUrl}housekeeping/delete-check-list`, {
          method: "POST",
          headers: {
