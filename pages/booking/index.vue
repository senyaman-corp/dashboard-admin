<template>
  <CardBaseCard title="Data Booking">
        ...
        >
          <template #column-8="props">
            ...
            >
              <li
                class="dropdown-item"
                v-if="props.rowData.status !== 'Checkout'"
              >
                <a
                  type="button"
                  class="btn btn-default"
                  @click="deleteBooking(props.rowData.id)"
                >
              ...
              <li class="dropdown-item">
                <a
                  type="button"
                  class="btn btn-default"
                  @click="viewDetail(props.rowData.id_booking_room)"
                >
              ...
              >
                <a
                  type="button"
                  class="btn btn-default"
                  @click="addAdditionalCharge(props.rowData.id_booking_room)"
                >
              ...
              >
                <a
                  type="button"
                  class="btn btn-default"
                  @click="extendsBooking(props.rowData.id_booking_room)"
                >
              ...
              >
                <button
                  class="btn btn-default"
                  target="_blank"
                  @click="checkout(props.rowData.id_booking_room)"
                >
  ...
  <WidgetModalPad :title="'Additional Charges'" :size="'modal-lg'">
    <form @submit.prevent="handleAddAdditionalCharge">
      <WidgetAdditionalCharge
        :addChargesData="addChargesData"
        @update:additionalCharges="updateAdditionalCharges"
      />
      <div class="d-flex justify-content-end">
        <ButtonBaseButton type="submit" variant="primary" class="mt-4 btn-md"
...
const { $bus, $dataTableOptions, $formatDateTime, $moment, $swal } =
  useNuxtApp();
...
const addAdditionalCharge = async (id) => {
  const { data, status } = await $fetch(
    `${config.public.baseUrl}bookings/additional-charges`,
    {
      method: "POST",
      headers: {
    ...
    .then(async (result) => {
      ...
      if (result.isConfirmed) {
        const response = await $fetch(
          `${config.public.baseUrl}bookings/cancel`,
          {
            method: "POST",
            headers: {
              Authorization: "Bearer " + authStore.getToken,
...
const exportBooking = async () => {
  let url = `${config.public.dashboardUrl}api/export-booking?token=${
    authStore.getToken
  }&start_date=${$moment(start_date.value).format(
    "YYYY-MM-DD"
...
const handleAddAdditionalCharge = async () => {
  ...
  const { data, status } = await $fetch(
    `${config.public.baseUrl}bookings/add-additional-charges`,
    {
      method: "POST",
      headers: {
        Authorization: "Bearer " + authStore.getToken,
...
const extendsBooking = async (id) => {
  $swal
    ...
    .then(async (result) => {
      if (result.isConfirmed) {
        const { data, status } = await $fetch(
          `${config.public.baseUrl}bookings/extends`,
          {
            method: "POST",
            headers: {
              Authorization: "Bearer " + authStore.getToken,
