<template>
  <CardBaseCard title="Data Pre Buy">
    <div class="d-flex justify-content-between align-items-center">
      <div v-if="dataPrebuy.length == 0">
        <ButtonBaseButton variant="primary" to="/pre-buy/add"
            ...
            >
              <li class="dropdown-item">
                <a
                  ...
                  @click="deletePrebuy(props.rowData.id)"
                >
...
const options = $dataTableOptions(
  config.public.baseUrl + "pre-buy/list",
  authStore.getToken
);
options.columnDefs = [
...
const deletePrebuy = async (id) => {
  const { data, status } = await $fetch(
    `${config.public.baseUrl}pre-buy/delete`,
    {
      method: "POST",
      headers: {
